...
namespace ForecastFavorApp_IntegrationTest
{
    ...
    public class IntegrationTest
    {
        ...
        //Asserting the Temperature, Condition, Pressure, Humidity, Cloud Cover, Feels Like, Wind Speed data from the api with the web in the tomorrow page
        [TestMethod]
        public async Task ApiDataAndWebDataForTomorrowPageIntegrationTest()
        {
            try
            {
                var apiKey = "bcea99f817174902bdc03259230311";
                var apiEndpoint = "http://api.weatherapi.com/v1/forecast.json?key=" + apiKey + "&q=sudbury&days=2&aqi=no&alerts=no";
                var apiResponse = await _httpClient.GetStringAsync(apiEndpoint);
                var apiData = JsonConvert.DeserializeObject<CurrentWeatherResponse>(apiResponse);
                var baseUrl = "https://localhost:7293/Weather";
                _webDriver.Navigate().GoToUrl(baseUrl);
